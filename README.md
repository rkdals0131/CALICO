...
일부 카메라/이미지 입력이 결손되더라도, 가능한 입력만으로 융합을 지속하고 출력이 중단되지 않도록 폴백 동작을 설계했습니다. (이 섹션은 구현 계획/사양입니다.)

핵심 동작
- LiDAR는 필수 입력입니다. LiDAR가 없으면 융합을 수행하지 않습니다.
- 카메라 검출은 “가용한 카메라만” 사용합니다. 결손 카메라는 자동으로 제외됩니다.
- 어떤 카메라도 해당 LiDAR 박스를 분류하지 못하면 해당 박스의 클래스는 `"Unknown"`으로 출력됩니다.
- 디버그 이미지가 없더라도 융합은 계속되며, 이미지가 있는 카메라만 오버레이를 게시합니다.

예상 파라미터(추가 예정)
- `min_cameras_for_fusion` (int, 기본 1): 융합에 필요한 최소 활성 카메라 수. 미만이면 출력 보류 또는 `Unknown` 처리 중 택일.
- `detection_stale_timeout` (double, 기본 0.5): 마지막 검출 수신 이후 이 시간 초과 시 해당 카메라를 “일시 결손”으로 간주하여 투표에서 제외.
- `allow_partial_cameras` (bool, 기본 true): 활성 카메라가 일부만 있어도 융합을 수행.
- `unknown_label` (string, 기본 "Unknown"): 분류 불가 시 사용할 레이블 명.
- (시각화) `viz_require_images` (bool, 기본 false): true여도 융합은 계속되며, 시각화만 제한.

시나리오별 처리
- 카메라 1만 활성: Cam1 검출만으로 투표/매칭, 미매칭 박스는 `Unknown`.
- 실행 중 카메라 2 드롭: `detection_stale_timeout` 경과 후 Cam2 제외, Cam1만으로 지속.
- YOLO 미발행 유지: 해당 카메라 제외. 모든 카메라 미발행이면 LiDAR만으로 `Unknown` 클래스 출력(옵션에 따라 보류 가능).
- 디버그 이미지 미발행: 해당 카메라 오버레이 생략. 나머지 카메라 이미지는 정상 게시.

로그 및 모니터링
- INFO: 활성/결손 카메라 목록과 원인(stale timeout 초과) 주기 보고.
- DEBUG: 카메라별 최근 수신 시각, 투표 결과, `Unknown` 할당 개수.

제약
- 최소 1대 이상의 카메라가 “일시적으로라도” 활성화되면 색상 분류 품질이 향상됩니다. 카메라 전무 시에는 위치만 있는 `Unknown` 출력이 됩니다.
- 파라미터 명과 기본값은 구현 시 약간 변동될 수 있으며, 반영 후 본 문서를 업데이트합니다.
